...
##準備時間
    execute if entity @a[predicate=glance:on_ground] as @a[predicate=glance:on_ground] at @s run clear @s feather


##弓矢
    execute as @a[gamemode=adventure] at @s store result score @s Znsi.Arrows if items entity @s container.* arrow
    ##リロード
        execute as @a[gamemode=adventure] at @s if score @s Znsi.Arrows < @s Znsi.MaxArrows run scoreboard players add @s Znsi.Reload 1
...
##MP関連    
    ...
    execute as @a at @s if score @s Znsi.MPRec = *** Znsi.MPRec run scoreboard players add @s Znsi.MP 1
    execute as @a at @s if score @s Znsi.MPRec = *** Znsi.MPRec run scoreboard players reset @s Znsi.MPRec
